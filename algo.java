...
//
// find prime O(nlog(logn))
static void mark_prime() throws IOException {
    boolean[] b=new boolean[1000000];
    Arrays.fill(b,true);
    for(int i=2;i*i<=1000000;i++){
        // note i*i<=1000000  after 1000 we are not iterating, after  
        //that all the true marked remaining no will be prime for sure
        if(b[i]) continue;
...
// dsu disjoint set union
 ...
 static void union_set(int x, int y){
    ...
    y=find_set(y);
    if(x==y) return;
    if(rank[x]<rank[y]){int k=x;x=y;y=k;}
    parent[y]=x;
